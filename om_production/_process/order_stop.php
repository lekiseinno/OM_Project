...
<?php
        session_start();
        $username_stop = $_SESSION['username_stop'];
    	...
    	if($rs['prod_order_no']!=''&&$rs['order_start_time']!=''&&$rs['order_end_time']==null){	
    		...
    		$sql = "UPDATE orderStartStopTime SET order_end_time = '$order_end_time',username_stop = '$username_stop' WHERE prod_order_no = '$prod_order_no' AND machine_id='$machine_id'";
    		$query = sqlsrv_query($connect, $sql) or die($sql);
